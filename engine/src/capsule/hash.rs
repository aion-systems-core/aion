...
use crate::ai::{
    canonical_events_for_hash, canonical_token_trace_for_hash, AICapsuleV1,
};
...
pub fn deterministic_capsule_hash(c: &AICapsuleV1) -> [u8; 32] {
    ...
    let graph: Value = serde_json::from_str(&canonical_graph_v2_json(&c.graph)).unwrap_or(json!({}));
