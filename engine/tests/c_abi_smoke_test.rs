...
fn c_abi_smoke_capsule_hash_symmetry_replay() {
    ...
    let rc = aion_capsule_deterministic_hash_hex(pc.as_ptr(), &mut hex);
    assert_eq!(rc, AION_OK);
    assert!(!hex.is_null());
    let s = unsafe { std::ffi::CStr::from_ptr(hex).to_string_lossy().into_owned() };
    assert_eq!(s.len(), 64);
    aion_free_string(hex);

    let mut sym: u8 = 0;
    let rc = aion_replay_symmetry_ok(pc.as_ptr(), &mut sym);
    assert_eq!(rc, AION_OK);
    assert!(sym <= 1, "symmetry flag must be bool-like");

    let mut rep: AionRunResult = unsafe { std::mem::zeroed() };
    let rc = aion_replay_capsule(pc.as_ptr(), &mut rep);
    assert_eq!(rc, AION_OK);
    aion_free_run_result(&mut rep);
...
fn c_abi_evidence_io_error_is_contract_json() {
    ...
    let mut ok: u8 = 0;
    let rc = aion_evidence_verify(missing.as_ptr(), &mut ok as *mut _);
