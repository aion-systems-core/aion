...
pub fn run_security_scans() -> SecurityScanResult {
    let mut scans = vec![
        SecurityScanConfig { scan: "sast".into(), mode: "required".into() },
        SecurityScanConfig { scan: "dependency_scanning".into(), mode: "required".into() },
        SecurityScanConfig { scan: "container_scanning".into(), mode: "optional".into() },
        SecurityScanConfig { scan: "secret_scanning".into(), mode: "required".into() },
    ];
...
mod tests {
    ...
    fn baseline_has_required_scans() {
        let r = run_security_scans();
        assert!(r.scans.iter().any(|s| s.scan == "sast" && s.mode == "required"));
    }
}
